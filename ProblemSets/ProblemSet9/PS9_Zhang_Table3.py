...
# Setting the inital values of all parameters from PS8
...
def z_fun(theta):
    ...
    alpha_k, rho, psi, sigma_z, phi0 = theta # Defining all the model parameters as theta.
    ...
    for i in range(sizez):  # loop over z
        for j in range(sizek):  # loop over k
            for k in range(sizek): # loop over k'
                if kvec[j]>(op[i,j]+(1 - delta) * kvec[j]): # in accordance with the inequality of argument in equation 11.
...
for i in range(n):
    z_new[i] = sim_markov(z, np.transpose(Pi), num_draws)  #correct z for each firm
...
# Defining the moments function to obtain the moments of simulated data
...
#bnds2=((0,0.8),(0,0.5),(0,0.05),(0,1),(0,0.005))
bnds2=((0,0.7),(0,0.2),(0,0.1),(0,1),(0,0.001))
